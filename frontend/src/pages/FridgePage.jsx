...
export default function FridgePage() {
  ...
  const grouped = useMemo(() => {
    return ingredients.reduce((acc, ing) => {
      const found = Object.entries(INGREDIENTS_DATA).find(
        ([cat, items]) => items.some(item => item.name === ing.name)
      );
  ...
  const handleIngredientClick = (ingredient) => {
    if (isSelecting) {
      ...
      setSelectedItems(prev =>
        already ? prev.filter(i => i.name !== ingredient.name) : [...prev, ingredient]
      );
    } else {
      ...
      const expireStr = `${exp.getFullYear()}-${fmt(exp.getMonth() + 1)}-${fmt(exp.getDate())}`;
  ...
  const handleStoredDateChange = (e) => {
    ...
    const newStored = `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
    ...
    const fmt = n => String(n).padStart(2, '0');
    const newExpire = `${base.getFullYear()}-${fmt(base.getMonth() + 1)}-${fmt(base.getDate())}`;
  ...
  const handleFreezeToggle = () => {
    ...
    if (nextFrozen) {
      ...
      const fmt = n => String(n).padStart(2, '0');
      setExpireDate(`${dt.getFullYear()}-${fmt(dt.getMonth() + 1)}-${fmt(dt.getDate())}`);
    } else {
      if (selectedIngredient) {
        ...
        setExpireDate(`${dt2.getFullYear()}-${fmt2(dt2.getMonth() + 1)}-${fmt2(dt2.getDate())}`);
      }
    }
  };
  ...
  const handleSaveDate = () => {
    ...
    const updated = ingredients.map(item =>
      item.name === selectedIngredient.name
        ? { ...item, expireDate }
        : item
    );
  ...
  const handleDeleteItem = () => {
    const filtered = ingredients.filter(item => item.name !== selectedIngredient.name);
  ...
  return (
    <div className={styles.pageContainer}>
        ...
        <section key={category}>
          ...
          <div className={styles.ingredientsList}>
            {items.map(ingredient => {
              const isSelected = selectedItems.some(i => i.name === ingredient.name);
              return (
                <div
                  key={ingredient.name}
                  className={`${styles.ingredientItem} ${isSelecting && isSelected ? styles.selectedItem : ''}`}
        ...
        <div className={styles.modalBackdrop}>
          <div className={styles.modalContent}>
            ...
            <p className={styles.modalSubtitle}>
              {selectedCategory} {isFrozen && <span style={{ color: 'skyblue' }}>냉동 보관</span>}
            </p>
      ...
      {/* 요리시작 버튼 */}
      <button className={styles.startButton} onClick={() => navigate('/recipe')}>
        요리시작
      </button>
    </div>
  );
